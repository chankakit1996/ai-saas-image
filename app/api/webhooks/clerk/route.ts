...
const createClerkUser = async (
  evt: WebhookEvent & { type: 'user.created' | 'user.updated' }
) => {
...
const deleteClerkUser = async (
  evt: WebhookEvent & { type: 'user.deleted' }
) => {
  const { id } = evt.data
...
export async function POST(req: Request) {
  ...
  if (!WEBHOOK_SECRET) {
    throw new Error('Clerk WEBHOOK_SECRET is not found')
  }
  ...
  console.log('Webhook headers:', JSON.stringify(evt.data))
